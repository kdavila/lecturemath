...
class CCStabilityEstimator:
    ...
    def finish_processing(self):
        ...
        print("Total CC merges tested: " + str(self.tempo_count))
    ...
    def rebuilt_binary_images(self):
        ...
        for frame_ccs in self.cc_idx_per_frame:
            binary = self.rebuilt_binary_frame(frame_ccs)
            rebuilt_frames.append(binary)

        return rebuilt_frames

    def rebuilt_binary_frame(self, frame_ccs):
    ...
    def compute_overlapping_stable_cc(self, stable_idxs, temporal_window):
        ...
        # using IntervalIndex to find all CCs that overlap in space through the video ...
        print("...preparing Interval Index ... ")
        main_index_x = IntervalIndex(True)
        main_index_y = IntervalIndex(True)
        for offset1 in range(n_stable):
            idx_cc1 = stable_idxs[offset1]
            cc1 = self.unique_cc_objects[idx_cc1]

            main_index_x.add(cc1.min_x, cc1.max_x + 1, idx_cc1)
            main_index_y.add(cc1.min_y, cc1.max_y + 1, idx_cc1)

        print("...finding pairwise matches ...")

        # compute other stable CC with matching regions
        set_x = set(main_index_x.find_matches(main_index_x))
        set_y = set(main_index_y.find_matches(main_index_y))

        # list of all pairs of CC with intersecting intervals in X and Y
        # .. only keep those pairs where first CC idx is smaller than second (avoid self matches and repetitions)
        merged = sorted([(idx_cc1, idx_cc2) for idx_cc1, idx_cc2 in set_x.intersection(set_y) if idx_cc1 < idx_cc2])

        print("...Total raw matches found: " + str(len(merged)))

        # for every overlapping CC found ... Check matches ...
        last_idx_cc1 = None
        for idx_cc1, idx_cc2 in merged:
            if last_idx_cc1 != idx_cc1:
                last_idx_cc1 = idx_cc1
                print("Processing: " + str(idx_cc1), end="\r")
    ...
    def compute_groups(self, stable_idxs, overlapping_cc, min_recall, t_fmeasure, t_time_IOU):
        n_stable = len(stable_idxs)

        debug_msg = "{0:d}\t{1:d}\t{2:d}\t{9:d}\t{3:.2f}\t{4:.2f}\t{5:d}\t{6:d}\t{7:d}\t{8:d}\t{10:.2f}"

        # Determine Groups of CC's that coexist in time and space and treat them as single units...
        # Note that here there is no guarantee of either CC1 or CC2 being sorted by recency ....
        cc_groups = []
        group_idx_per_cc = {}
        # tempo_all_recall = []
        for offset1 in range(n_stable):
            ...
            for idx_cc2, recall, precision in overlapping_cc[idx_cc1]:
                ...
                if cc2_t_start < cc1_t_end and cc1_t_start < cc2_t_end:
                    time_INT = (min(cc1_t_end, cc2_t_end) - max(cc1_t_start, cc2_t_start))
                    time_UNION = (max(cc1_t_end, cc2_t_end) - min(cc1_t_start, cc2_t_start))
                    time_IOU = time_INT / time_UNION
                else:
                    time_IOU = 0.0
                ...
                if fmeasure < t_fmeasure and time_IOU < t_time_IOU:
                    # it does not meet the criteria for merging ... skip ....
                    continue
                """
                # check for min recall
                if recall < min_recall:
                    # skip this pair .... do not merge
                    continue

                # print("{0:d}\t{1:d}\t{2:.4f}\t{3:.4f}\t{4:.4f}".format(idx_cc1, idx_cc2, recall, precision, fmeasure))
                # tempo_all_recall.append(recall)
                ...
                if idx_cc2 not in group_idx_per_cc:
                    ...
                    """
                    print(debug_msg.format(idx_cc1, idx_cc2, group_idx, recall, precision, cc1_t_start, cc1_t_end,
                                           cc2_t_start, cc2_t_end, - 1, time_IOU))
                    """
                else:
                    ...
                    """
                    print(debug_msg.format(idx_cc1, idx_cc2, group_idx, recall, precision, cc1_t_start, cc1_t_end,
                                           cc2_t_start, cc2_t_end, other_group_idx, time_IOU))
                    """
        ...
        # print("-> distribution of recall for grouped CC")
        # print(np.histogram(np.array(tempo_all_recall) * 10, range(0, 11), density=True))
    ...
    def compute_conflicting_groups(self, stable_idxs, all_overlapping_cc, n_groups, group_idx_per_cc):
        ...
        for offset1 in range(n_stable):
            ...
            for idx_cc2, matched_pixels, size_cc2, size_cc1 in all_overlapping_cc[idx_cc1]:
                ...
                if idx_cc1 < idx_cc2:
                    cc2 = self.unique_cc_objects[idx_cc2]
                    cc2_box_area = cc2.getBoxArea()

                    unmatched_pixels = size_cc1 + size_cc2 - matched_pixels * 2

                    area_intersection = cc1.getOverlapArea(cc2)
                    area_union = cc1_box_area + cc2_box_area - area_intersection
                    ...
                    if group_idx1 != group_idx2:
                        ...
                        if group_idx2 in conflicts[group_idx1]:
                            ...
                            conflicts[group_idx1][group_idx2]["area_union"] += area_union
                            conflicts[group_idx1][group_idx2]["area_intersection"] += area_intersection
                        else:
                        ...
                        if group_idx1 in conflicts[group_idx2]:
                            ...
                            conflicts[group_idx2][group_idx1]["area_union"] += area_union
                            conflicts[group_idx2][group_idx1]["area_intersection"] += area_intersection
                        else:
