...
class UniqueCCGroup:
    ...
    def GroupsFromXML(keyframes, xml_filename):
        ...
        for group_xml in groups_xml_roots:
            ...
            for kf_offset, group_cc_xml in enumerate(group_cc_xml_roots):
                if group_cc_xml.text is None:
                    # empty CC ID (the group continues, but has no CC on this frame)
                    valid_group_ids.append(None)
                else:
            ...
            if len(valid_group_ids) > 0:
                ...
                for kf_offset in range(1, len(valid_group_ids)):
                    if valid_group_ids[kf_offset] is not None:
                        # add to the group
                        ...
                        # add empty reference on the group
                        # no CC from this key-frame is referenced ....
                        new_group.cc_refs.append(None)
    ...
    def Split(original, split_frame):
        ...
        if offset_split <= 0 or offset_split >= len(original.cc_refs):
            ...
            # remove any extra empty CC's added to merge gaps ...
            while original.cc_refs[-1] is None:
                del original.cc_refs[-1]
