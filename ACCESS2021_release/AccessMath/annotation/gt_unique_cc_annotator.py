...
class GTUniqueCCAnnotator(Screen):
    ...
    ModeMatch_SourceSelection = 1
    ModeMatch_TransformSelection = 2
    ModeMatch_RegionSelection = 3
    ModeMatch_Matching = 4
    ...
    def __init__(self, size, db_name, lecture_title, output_path):
        ...
        self.matching_source = None

        self.matching_projection_H = None
        self.matching_projection_src = None
        self.matching_projection_dst = None
        ...
        # ===============================================
        # Panel with matching parameters for step 1 (Source Key-frame to Match)
        self.container_matching_source = ScreenContainer("container_matching_source", (container_width, 150),
                                                         general_background)
        self.container_matching_source.position = (self.container_view_buttons.get_left(),
                                                   self.container_view_buttons.get_bottom() + 5)
        self.elements.append(self.container_matching_source)

        self.lbl_matching_source = ScreenLabel("lbl_matching_source", "Source Keyframe: [x]", 21, 290, 1)
        self.lbl_matching_source.position = (5, 5)
        self.lbl_matching_source.set_background(general_background)
        self.lbl_matching_source.set_color(text_color)
        self.container_matching_source.append(self.lbl_matching_source)

        self.btn_matching_source_prev = ScreenButton("btn_matching_source_prev", "Prev", 21, 130)
        self.btn_matching_source_prev.set_colors(button_text_color, button_back_color)
        self.btn_matching_source_prev.position = (10, self.lbl_matching_source.get_bottom() + 10)
        self.btn_matching_source_prev.click_callback = self.btn_matching_source_prev_click
        self.container_matching_source.append(self.btn_matching_source_prev)

        self.btn_matching_source_next = ScreenButton("btn_matching_source_next", "Next", 21, 130)
        self.btn_matching_source_next.set_colors(button_text_color, button_back_color)
        self.btn_matching_source_next.position = (
            self.container_matching_source.width - self.btn_matching_source_next.width - 10,
            self.lbl_matching_source.get_bottom() + 10)
        self.btn_matching_source_next.click_callback = self.btn_matching_source_next_click
        self.container_matching_source.append(self.btn_matching_source_next)
        self.container_matching_source.visible = False

        # ===============================================
        # Panel with matching parameters for step 4 (Matching Strictness)
        ...
        quad_points = np.array([[10.0, 10.0], [50.0, 10.0], [50.0, 50.0], [10.0, 50.0]])
        self.canvas_select.add_polygon_element("projection_quad", quad_points)
        self.canvas_select.elements["projection_quad"].visible = False
    ...
    def update_current_view(self, resized=False):
        if (self.edition_mode == GTUniqueCCAnnotator.ModeMatch_SourceSelection or
            self.edition_mode == GTUniqueCCAnnotator.ModeMatch_TransformSelection or
            self.edition_mode == GTUniqueCCAnnotator.ModeMatch_RegionSelection or
        ...
        if self.edition_mode == GTUniqueCCAnnotator.ModeMatch_Matching:
            ...
            for recall, precision, prev_cc_str_id, prev_cc, curr_cc in self.matching_scores:
    ...
    def update_view_scale(self, new_scale):
        ...
        # if projection quad is active ...
        if self.edition_mode == GTUniqueCCAnnotator.ModeMatch_TransformSelection:
            self.canvas_select.elements["projection_quad"].points *= scale_factor
    ...
    def greedy_matching_scores(self):
        ...
        print("Total candidates (Current-KF): " + str(len(curr_ccs)))
        if len(filtered_ccs) != len(curr_ccs):
            print("Total candidates not previously matched (Current-KF): " + str(len(filtered_ccs)))

            curr_ccs = filtered_ccs

        # identify CC's from source frame within selected region (containment)
        prev_kf = self.keyframe_annotations[self.matching_source]

        # prepare a image with labeled CCs based on original CCs
        dst_size = (prev_kf.binary_image.shape[0], prev_kf.binary_image.shape[1])
        labeled_img = np.zeros(dst_size, dtype=np.int32)
        for idx, cc in enumerate(prev_kf.binary_cc):
            cc_region = labeled_img[cc.min_y:cc.max_y + 1, cc.min_x:cc.max_x + 1]
            cc_region[cc.img > 0] = idx + 1

        # warp the image and get its CC's ....
        warped_labeled = cv2.warpPerspective(labeled_img, self.matching_projection_H, (dst_size[1], dst_size[0]), flags=cv2.INTER_NEAREST)
        warped_ccs = Labeler.extractConnectedComponents(warped_labeled, False, True)

        # create an index for new CC's based on original CC ids ....
        prev_ccs = {}
        for idx, cc in enumerate(warped_ccs):
            # only index the CC's in the current region of interest ....
            # .... which have not been matched to anything on this frame or on later frames ....
            original_id = prev_kf.binary_cc[idx].strID()
            if (self.cc_group[self.matching_source][original_id].lastFrame() < self.selected_keyframe and
                rect_x <= cc.min_x and cc.max_x <= rect_x + rect_w and
                rect_y <= cc.min_y and cc.max_y <= rect_y + rect_h):
                prev_ccs[original_id] = ConnectedComponent.ShallowCopy(cc)

        print("Total candidates (Source-KF): " + str(len(prev_ccs)))
        ...
        for curr_cc_str_id in curr_ccs:
            ...
            for prev_cc_str_id in prev_ccs:
                ...
                if (curr_cc.min_x < prev_cc.max_x and prev_cc.min_x < curr_cc.max_x and
                    ...
                    # ...get overlap scores (recall and precision)
                    recall, precision = curr_cc.getOverlapFMeasure(prev_cc, False, False)

                    # save .... including the string ID of the original un-warped CC from source frame ....
                    all_matches.append((recall, precision, prev_cc_str_id, prev_cc, curr_cc))

                    # print((recall, precision, prev_cc_str_id, prev_cc.strID(), curr_cc_str_id))
        ...
        matched_curr_ids = {}
        matched_prev_ids = {}
        for recall, precision, prev_cc_str_id, prev_cc, curr_cc in all_matches:
            ...
            # otherwise, greedily accept match ....
            #print((prev_cc, curr_cc))
            self.matching_scores.append((recall, precision, prev_cc_str_id, prev_cc, curr_cc))
            # marked as matched ...
    ...
    def btn_confirm_accept_click(self, button):
        if self.edition_mode == GTUniqueCCAnnotator.ModeMatch_SourceSelection:
            # matching source selected ... move to the next stage ...
            self.prepare_projection_quad(100)
            self.set_editor_mode(GTUniqueCCAnnotator.ModeMatch_TransformSelection)
            self.update_current_view(False)

        elif self.edition_mode == GTUniqueCCAnnotator.ModeMatch_TransformSelection:
            # matching projection selected ... move to the next stage ...
            self.prepare_selection_rectangle(40)
            self.set_editor_mode(GTUniqueCCAnnotator.ModeMatch_RegionSelection)
            self.update_current_view(False)

        elif self.edition_mode == GTUniqueCCAnnotator.ModeMatch_RegionSelection:
            ...
            # accept matches
            for recall, precision, prev_id, prev_cc, curr_cc in self.matching_scores:
                if precision >= self.matching_min_precision and recall >= self.matching_min_recall:
                    # merge current group with previous group ...
                    prev_group = self.cc_group[self.matching_source][prev_id]

                    # there could be a gap between source and target ..... fill it!
                    while prev_group.lastFrame() + 1 < self.selected_keyframe:
                        prev_group.cc_refs.append(None)
                    ...
                    for kf_offset, cc in enumerate(curr_group.cc_refs):
                        if cc is not None:
                            ...
                            # move element from current group to the group of cc from source keyframe
    ...
    def get_view_rectangle(self, margin):
    ...
    def prepare_selection_rectangle(self, margin):
        # Default selection rectangle is relative to current view
        rect_x, rect_y, rect_w, rect_h = self.get_view_rectangle(margin)
    ...
    def prepare_projection_quad(self, margin):
        # Default projection quad starts with rectangle relative to current view
        rect_x, rect_y, rect_w, rect_h = self.get_view_rectangle(margin)

        new_points = np.array([[rect_x, rect_y],
                               [rect_x + rect_w, rect_y],
                               [rect_x + rect_w, rect_y + rect_h],
                               [rect_x, rect_y + rect_h]])
        self.canvas_select.elements["projection_quad"].update(new_points)

        self.matching_projection_src = new_points / self.view_scale
        self.matching_projection_dst = new_points / self.view_scale
    ...
    def btn_matches_add_click(self, button):
        ...
        # by default ... start with previous key-frame
        self.update_matching_source(self.selected_keyframe - 1)
        # ... and assume no perspective deformation
        self.matching_projection_H = np.identity(3)
        # self.prepare_selection_rectangle(40)
        self.set_editor_mode(GTUniqueCCAnnotator.ModeMatch_SourceSelection)
    ...
    def update_matching_image(self):
        prev_binary = self.keyframe_annotations[self.matching_source].binary_image[:, :, 0]
        curr_binary = self.keyframe_annotations[self.selected_keyframe].binary_image[:, :, 0]

        # adding current perspective distortion ...
        dst_size = (prev_binary.shape[1], prev_binary.shape[0])
        projected_prev = cv2.warpPerspective(prev_binary, self.matching_projection_H, dst_size)

        self.base_matching = Visualizer.combine_bin_images_w_disp(curr_binary, projected_prev, 0, 0, 0)
    ...
    def set_editor_mode(self, new_mode):
        ...
        show_view_buttons = (new_mode not in [GTUniqueCCAnnotator.ModeMatch_SourceSelection,
                                              GTUniqueCCAnnotator.ModeMatch_TransformSelection,
                                              GTUniqueCCAnnotator.ModeMatch_RegionSelection,
                                              GTUniqueCCAnnotator.ModeMatch_Matching])

        self.btn_view_raw.visible = show_view_buttons
        self.btn_view_gray.visible = show_view_buttons
        self.btn_view_binary.visible = show_view_buttons
        self.btn_view_colored.visible = show_view_buttons

        self.container_confirm_buttons.visible = new_mode in [GTUniqueCCAnnotator.ModeMatch_SourceSelection,
                                                              GTUniqueCCAnnotator.ModeMatch_TransformSelection,
                                                              GTUniqueCCAnnotator.ModeMatch_RegionSelection,
                                                              GTUniqueCCAnnotator.ModeMatch_Matching,
                                                              GTUniqueCCAnnotator.ModeMatch_Remove,
                                                              GTUniqueCCAnnotator.ModeExitConfirm]

        if new_mode == GTUniqueCCAnnotator.ModeMatch_SourceSelection:
            self.lbl_confirm_message.set_text("Selecting Source Key-frame")
        elif new_mode == GTUniqueCCAnnotator.ModeMatch_TransformSelection:
            self.lbl_confirm_message.set_text("Aligning Source and Target")
        elif new_mode == GTUniqueCCAnnotator.ModeMatch_RegionSelection:
        ...
        # by default, lock canvas and hide objects ...
        self.canvas_select.locked = True
        self.canvas_select.elements["selection_rectangle"].visible = False
        self.canvas_select.elements["projection_quad"].visible = False

        if new_mode == GTUniqueCCAnnotator.ModeMatch_TransformSelection:
            # show projection quad ...
            self.canvas_select.locked = False
            self.canvas_select.elements["projection_quad"].visible = True
        elif new_mode == GTUniqueCCAnnotator.ModeMatch_RegionSelection or new_mode == GTUniqueCCAnnotator.ModeMatch_Remove:
            # show rectangle
            self.canvas_select.locked = False
        ...
        self.container_matching_source.visible = (new_mode == GTUniqueCCAnnotator.ModeMatch_SourceSelection)
        self.container_matching_strictness.visible = (new_mode == GTUniqueCCAnnotator.ModeMatch_Matching)

    def btn_matching_source_prev_click(self, button):
        if self.matching_source > 0:
            self.update_matching_source(self.matching_source - 1)
            self.update_matching_image()
            self.update_current_view(False)

    def btn_matching_source_next_click(self, button):
        if self.matching_source < self.selected_keyframe - 1:
            self.update_matching_source(self.matching_source + 1)
            self.update_matching_image()
            self.update_current_view(False)

    def update_matching_source(self, new_source):
        self.matching_source = new_source
        self.lbl_matching_source.set_text("Source Keyframe: {0:d}".format(new_source + 1))

    def update_projection(self):
        self.matching_projection_H, mask = cv2.findHomography(self.matching_projection_src, self.matching_projection_dst)

    def canvas_object_edited(self, canvas, edited_object):
        if edited_object == "projection_quad":
            # update the projection ....
            self.matching_projection_dst = self.canvas_select.elements["projection_quad"].points / self.view_scale
            self.update_projection()
