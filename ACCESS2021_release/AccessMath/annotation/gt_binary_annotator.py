...
class GTBinaryAnnotator(Screen):
    ...
    def __init__(self, size, raw_input, partial_gt, initial_K, parent_screen=None):
        ...
        self.large_mode = self.height > 900
        ...
        self.grid_point_offset_last_time = None
        ...
        grid_buttons_height = 280 if self.small_mode else (450 if self.large_mode else 350)
        self.container_grid_buttons = ScreenContainer("container_grid_buttons", (300, grid_buttons_height), back_color=general_background)
        self.container_grid_buttons.position = (self.container_point_buttons.get_left(),
                                                self.container_point_buttons.get_bottom() + 5)
        ...
        self.lbl_median_blur_K = ScreenLabel("lbl_median_blur_K", "BG Removal. Median K: " + str(self.median_K), font_size, 290, 1)
        self.lbl_median_blur_K.position = (5, self.btn_grid_noise_inc.get_bottom() + short_gap)
        self.lbl_median_blur_K.set_background(general_background)
        self.lbl_median_blur_K.set_color(text_color)
        self.container_grid_buttons.append(self.lbl_median_blur_K)

        # note that this value should be used as a multiple to ensure that only valid odd values are selected
        self.scroll_median_blur_K = ScreenHorizontalScroll("scroll_median_blur_K", 1, 25, 1, 2)
        self.scroll_median_blur_K.position = (5, self.lbl_median_blur_K.get_bottom() + short_gap)
        self.scroll_median_blur_K.width = 270
        self.scroll_median_blur_K.scroll_callback = self.scroll_median_blur_K_change
        self.container_grid_buttons.append(self.scroll_median_blur_K)
        ...
        self.lbl_smoothing_color.position = (5, self.scroll_median_blur_K.get_bottom() + short_gap)
        ...
        self.scroll_smoothing_color = ScreenHorizontalScroll("scroll_smoothing_color", 0, 50, 0, 10)
        self.scroll_smoothing_color.position = (5, self.lbl_smoothing_color.get_bottom() + short_gap)
        self.scroll_smoothing_color.width = 270
        ...
        self.scroll_smoothing_space = ScreenHorizontalScroll("scroll_smoothing_space", 1, 50, 0, 10)
        self.scroll_smoothing_space.position = (5, self.lbl_smoothing_space.get_bottom() + short_gap)
        self.scroll_smoothing_space.width = 270
        ...
        self.container_state_buttons = ScreenContainer("container_state_buttons", (300, 160), general_background)
        self.container_state_buttons.position = (self.container_view_buttons.get_left(),
                                                 self.height - self.container_state_buttons.height - 10)
        self.elements.append(self.container_state_buttons)

        self.btn_dark = ScreenButton("btn_dark", "Dark/Light", font_size, button_width)
        self.btn_dark.set_colors(button_text_color, button_back_color)
        self.btn_dark.position = (button_left, 5)
        self.btn_dark.click_callback = self.btn_dark_click
        self.container_state_buttons.append(self.btn_dark)
        ...
        self.btn_undo.position = (button_3_left, self.btn_dark.get_bottom() + short_gap)
        ...
        self.btn_redo.position = (button_3_middle, self.btn_dark.get_bottom() + short_gap)
        ...
        if self.parent_screen is None:
            ...
            self.btn_save.position = (button_3_right, self.btn_dark.get_bottom() + short_gap)
            ...
            self.btn_exit.position = (button_left, self.btn_undo.get_bottom() + short_gap)
            ...
            self.btn_return_cancel.position = (int(self.container_state_buttons.width * 0.25) - self.btn_return_cancel.width / 2,
                                               self.btn_undo.get_bottom() + short_gap)
            ...
            self.btn_return_accept.position = (button_2_right, self.btn_undo.get_bottom() + short_gap)
    ...
    def btn_confirm_accept_click(self, button):
        if self.editor_mode == GTBinaryAnnotator.ModeConfirmExit:
            ...
            if self.finished_callback is not None:
                # cancel ...
                self.finished_callback(False, None, None, None, self.is_dark)
    ...
    def update_image_K(self, new_K, add_to_undo_stack=True):
        ...
        self.lbl_knn.set_text("K-NN K: " + str(self.image_K))
    ...
    def update_base_images(self, starting_step):
        if starting_step == -1:
            ...
            if self.median_K > 1:
                blurred_bg = cv2.medianBlur(color_source, self.median_K)
                subtracted = color_source.astype(np.int32) - blurred_bg.astype(np.int32)
                ...
                # invert, to keep background white and writing darker ....
                self.base_img_gray = 255 - np.max(subtracted, axis=2)
            ...
            if self.median_K == 1 and self.is_dark:
                # invert the color ...
                self.base_img_gray = 255 - self.base_img_gray
                self.base_img_enhanced_gray = 255 - self.base_img_enhanced_gray
    ...
    def generate_initial_gt_adaptive_otsu(self, image_cut, rows, cols):
        ...
        # if bilateral filtering is used ...
        if self.smoothing_sigma_color > 0:
            image_cut = cv2.bilateralFilter(image_cut, -1, self.smoothing_sigma_color, self.smoothing_sigma_space)

        if self.median_K > 1:
            blurred_bg = cv2.medianBlur(image_cut, self.median_K)
            subtracted = image_cut.astype(np.int32) - blurred_bg.astype(np.int32)
            ...
            # invert, to keep background white and writing darker ....
            tempo_gray = 255 - np.max(subtracted, axis=2)
            tempo_gray = tempo_gray.astype(np.uint8)
        else:
    ...
    def btn_grid_offset_dec_click(self, button):
        self.time_sensitive_apply_grid_offset(-1)

    def btn_grid_offset_inc_click(self, button):
        self.time_sensitive_apply_grid_offset(1)

    def time_sensitive_apply_grid_offset(self, sign):
        current_time = time.time()

        if self.grid_point_offset_last_time is None or (current_time - self.grid_point_offset_last_time) > 0.30:
            # first time or after more than X amount of time ... small increment/decrement
            self.apply_offset_all_points(sign)
        else:
            # done quicker, increase intensity
            self.apply_offset_all_points(sign * 5)

        self.grid_point_offset_last_time = current_time
    ...
    def btn_return_accept_click(self, button):
        if self.finished_callback is not None:
            # accept ...
            self.finished_callback(True, self.base_img_gray, self.base_img_thresholds, self.base_img_binary, self.is_dark)
    ...
    def scroll_median_blur_K_change(self, scroll):
        self.median_K = self.scroll_median_blur_K.value * 2 - 1

        self.lbl_median_blur_K.set_text("BG Removal. Median K: " + str(self.median_K))

        # update from gray-scale portion
        self.grid_changed_update(-1, False)

    def set_preprocessing_parameters(self, is_dark, median_K, sigma_space, sigma_color):
        self.is_dark = is_dark
        self.median_K = median_K
        self.smoothing_sigma_space = sigma_space
        self.smoothing_sigma_color = sigma_color

        # update UI
        # ... scroll bars ... setting the value directly (non-ideal, but avoids triggering redundant callbacks)
        self.scroll_median_blur_K.value = int((self.median_K + 1) / 2)
        self.scroll_smoothing_space.value = self.smoothing_sigma_space
        self.scroll_smoothing_color.value = self.smoothing_sigma_color
        # ... and the labels ....
        self.lbl_median_blur_K.set_text("BG Removal. Median K: " + str(self.median_K))
        self.lbl_smoothing_color.set_text("Smoothing Sigma Color: " + str(self.smoothing_sigma_color))
        self.lbl_smoothing_space.set_text("Smoothing Sigma Space: " + str(self.smoothing_sigma_space))

        # update images and binarization ....
        self.grid_changed_update(-1, False)
        self.update_point_grid(False)
    ...
    def main_key_up(self, scancode, key, unicode):
        ...
        if key == 269:
            ...
            elif self.editor_mode == GTBinaryAnnotator.ModeEdition:
                if self.parent_screen is None:
                    self.btn_save_click(None)
                else:
    ...
    def btn_dark_click(self, button):
        # toggle ..
